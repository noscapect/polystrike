...
function flatArenaCenterWeight(x, z) {
  if (currentMap === 5) {
    ...
    return Math.max(0, 1 - Math.min(1, d / 38));
  }
  if (currentMap === 6) {
    const d = Math.hypot(Number(x) || 0, Number(z) || 0);
    return Math.max(0, 1 - Math.min(1, d / 30));
  }
  return 0;
}
...
function getNavOutEdgeCapForNode(node) {
  ...
  if ((currentMap === 5 || currentMap === 6) && node) {
    const center = flatArenaCenterWeight(node.x, node.z);
    if (center > 0.55) cap += 3;
    else if (center > 0.08) cap += 2;
  }
  return cap;
}
...
function botShoot(bot) {
  ...
  const phase = (performance.now() * 0.001) + (bot.aimPhase || 0);
  const drift = (bot.aimDrift || 0.06) * human.driftMul;
  const jitterX = Math.sin(phase * (1.8 + (bot.aimTempo || 0.5))) * drift;
  const jitterY = Math.cos(phase * (1.35 + (bot.aimTempo || 0.5))) * drift * 0.4;
  const jitterZ = Math.sin(phase * (2.2 + (bot.aimTempo || 0.5))) * drift;
  dir.x += jitterX + (Math.random() - 0.5) * spread;
  dir.y += jitterY + (Math.random() - 0.5) * spread * 0.4;
  dir.z += jitterZ + (Math.random() - 0.5) * spread;
  dir.normalize();
...
function updateBots(dt) {
  ...
  for (const bot of bots) {
    ...
    if (currentMap !== 2 && MAP_BOUNDS && MAP_BOUNDS[currentMap]) {
      ...
      const threshold = currentMap === 6 ? 0.46 : 0.82; // DM-Fletcher uses an aggressive 0.46 threshold
      
      if (edge01 > threshold) {
        const h = Math.hypot(bp.x, bp.z) || 1;
        const pullFactor = currentMap === 6 ? 1.5 : 3.5;
        const pullStr = currentMap === 6 ? 0.58 : 0.75;
        const pull = Math.min(0.85, (edge01 - threshold) * pullFactor);
