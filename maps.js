...
function addDecoBox(x, y, z, w, h, d, mat) {
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat);
  mesh.position.set(x, y, z);
  mesh.castShadow = true; mesh.receiveShadow = true;
  sceneAddArena(mesh);
...
function buildArena() {
  ...
  [[0, 0.15, -57, 80, 0.3, 0.4], [0, 0.15, 57, 80, 0.3, 0.4],
   [-57, 0.15, 0, 0.4, 0.3, 80], [57, 0.15, 0, 0.4, 0.3, 80]].forEach(a => addDecoBox(...a, matTrim));

  [[-20, 0.02, 0, 0.35, 0.08, 40], [20, 0.02, 0, 0.35, 0.08, 40],
   [0, 0.02, -20, 40, 0.08, 0.35], [0, 0.02, 20, 40, 0.08, 0.35]].forEach(a => addDecoBox(...a, matGlow));

  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2;
    const px = Math.cos(a) * 14, pz = Math.sin(a) * 14;
    addBox(px, 3.5, pz, 2, 7, 2, matPillar);
    addDecoBox(px, 7.25, pz, 2.6, 0.5, 2.6, matTrim);
  }
  ...
  [[9, 4.55, 0, 0.15, 0.08, 18], [-9, 4.55, 0, 0.15, 0.08, 18],
   [0, 4.55, 9, 18, 0.08, 0.15], [0, 4.55, -9, 18, 0.08, 0.15]].forEach(a => addDecoBox(...a, matTech));
...
function buildDeck16() {
  ...
  [[0, 4.56, 13.1, 50, 0.1, 0.15], [0, 4.56, -13.1, 50, 0.1, 0.15],
   [25.1, 4.56, 0, 0.15, 0.1, 26], [-25.1, 4.56, 0, 0.15, 0.1, 26]].forEach(a => addDecoBox(...a, matGlow));
  ...
  [[0, 6.15, 4.1, 100, 0.3, 0.3], [0, 6.15, -4.1, 100, 0.3, 0.3]].forEach(a => addDecoBox(...a, matTrim));
  ...
  [[34, 6.2, -25], [-34, 6.2, -25], [34, 6.2, 25], [-34, 6.2, 25]].forEach(([x, y, z]) => {
    addDecoBox(x, y, z, 2.5, 1.8, 0.15, matGlow);
  });
  ...
  [[-49, 3, -20], [-49, 3, 20], [49, 3, -20], [49, 3, 20],
   [-20, 3, -39], [20, 3, -39], [-20, 3, 39], [20, 3, 39]].forEach(([x, y, z]) => {
    addDecoBox(x, y, z, 0.35, 1.0, 1.4, matWarn);
  });
...
function buildPhobos() {
  ...
  [[0, 5.36, 18.1, 36, 0.12, 0.9], [0, 5.36, -18.1, 36, 0.12, 0.9],
   [18.1, 5.36, 0, 0.9, 0.12, 36], [-18.1, 5.36, 0, 0.9, 0.12, 36]].forEach(a => addDecoBox(...a, matHaz));
  ...
  [[-52, 2.5, -25], [-52, 2.5, 25], [52, 2.5, -25], [52, 2.5, 25],
   [-25, 2.5, -52], [25, 2.5, -52], [-25, 2.5, 52], [25, 2.5, 52]].forEach(([x, y, z]) => {
    addDecoBox(x, y, z, 0.4, 1.2, 0.9, matHaz);
  });
...
function buildCodex() {
  ...
  [[4, 3.8, 4.1, 30, 0.1, 0.2], [4, 3.8, -4.1, 30, 0.1, 0.2]].forEach(a => addDecoBox(...a, matTrim));
  ...
  addDecoBox(-14, 0.56, 24, 16, 0.12, 10, matHazard);
  addDecoBox(20, 0.56, -24, 16, 0.12, 10, matHazard);
  addDecoBox(-44, 1.55, 47.2, 16, 0.12, 1.6, matConcrete);
  addDecoBox(50, 4.55, 5.6, 12, 0.12, 1.6, matConcrete);
...
function buildFactory() {
  ...
  [[0, 4.76, -14.1, 84, 0.12, 0.9], [0, 4.76, -25.9, 84, 0.12, 0.9],
   ...
   [21.9, 4.76, 0, 0.9, 0.12, 44], [34.1, 4.76, 0, 0.9, 0.12, 44]]
    .forEach(([x, y, z, w, h, d]) => addDecoBox(x, y, z, w, h, d, matHaz));
...
// ── Map 6: DM-Fletcher — HOTEL LOBBY
function buildFletcher() {
  const tMarble = texWhiteMarble(); tMarble.repeat.set(16, 12);
  const tWood = texConcreteCool(); tWood.repeat.set(6, 6);
  const tFabric = texConcreteCool(); tFabric.repeat.set(8, 8);
  const tPlant = texGothicWall(); tPlant.repeat.set(2, 2);

  const matMarble = new THREE.MeshStandardMaterial({ color: 0xf5f0e6, roughness: 0.15, metalness: 0.2, map: tMarble });
  const matDarkWood = new THREE.MeshStandardMaterial({ color: 0x4a3219, roughness: 0.65, metalness: 0.04, map: tWood });
  const matCarpet = new THREE.MeshStandardMaterial({ color: 0x8b0000, roughness: 0.95, metalness: 0.0, map: tFabric });
  const matGoldTrim = new THREE.MeshStandardMaterial({ color: 0xd4af37, roughness: 0.28, metalness: 0.85, emissive: new THREE.Color(0x604010), emissiveIntensity: 0.3 });
  const matWallPanel = new THREE.MeshStandardMaterial({ color: 0xe8e2d2, roughness: 0.55, metalness: 0.02 });
  const matChandelier = new THREE.MeshStandardMaterial({ color: 0xfff8e0, emissive: new THREE.Color(0xffe8a0), emissiveIntensity: 2.0, roughness: 0.1 });
  const matGlass = new THREE.MeshStandardMaterial({ color: 0xaaccff, roughness: 0.1, metalness: 0.8, transparent: true, opacity: 0.4 });
  const matPlant = new THREE.MeshStandardMaterial({ color: 0x2e4a22, roughness: 0.8, metalness: 0.0, map: tPlant });

  // Polished marble floor (Ground Level)
  addBox(0, 0.55, 0, 116, 1.1, 100, matMarble);

  // Grand outer walls with paneling (Taller now)
  [[-58, 14, 0, 4, 28, 104], [58, 14, 0, 4, 28, 104], [0, 14, -50, 120, 28, 4], [0, 14, 50, 120, 28, 4]]
    .forEach(a => addBox(...a, matWallPanel));

  // Ceiling and Skylight
  addBox(0, 28, -32, 120, 2, 40, matWallPanel);
  addBox(0, 28, 32, 120, 2, 40, matWallPanel);
  addBox(-40, 28, 0, 40, 2, 24, matWallPanel);
  addBox(40, 28, 0, 40, 2, 24, matWallPanel);
  addBox(0, 28, 0, 42, 2.5, 26, matGoldTrim); // Frame
  addBox(0, 28, 0, 38, 3, 22, matGlass); // Glass

  // Mezzanine balcony (U-shape around the back and sides)
  addBox(0, 9.25, -36, 112, 1.5, 24, matMarble); // Back Mezzanine
  addBox(-44, 9.25, 0, 24, 1.5, 48, matMarble); // Left Mezzanine
  addBox( 44, 9.25, 0, 24, 1.5, 48, matMarble); // Right Mezzanine

  // Mezzanine railings (Glass with gold trim)
  addBox(0, 11, -24.5, 64, 2, 0.5, matGlass); // Back railing
  addBox(0, 12.1, -24.5, 64, 0.4, 0.8, matGoldTrim);
  addBox(-32.5, 11, 0, 0.5, 2, 48, matGlass); // Side railings
  addBox(-32.5, 12.1, 0, 0.8, 0.4, 48, matGoldTrim);
  addBox( 32.5, 11, 0, 0.5, 2, 48, matGlass);
  addBox( 32.5, 12.1, 0, 0.8, 0.4, 48, matGoldTrim);

  // Grand Twin Staircases leading to Mezzanine
  for (let i = 0; i < 16; i++) {
    const stepHeight = 0.5;
    const stepDepth = 1.0;
    const y = 1.1 + i * stepHeight + stepHeight/2;
    const z = -8.5 - i * stepDepth;
    // Left stair
    addBox(-26, y, z, 12, stepHeight, stepDepth, matMarble);
    // Right stair
    addBox( 26, y, z, 12, stepHeight, stepDepth, matMarble);
  }
  // Staircase railings
  addBox(-20.5, 6, -16, 0.5, 10, 16, matGlass);
  addBox(-31.5, 6, -16, 0.5, 10, 16, matGlass);
  addBox( 20.5, 6, -16, 0.5, 10, 16, matGlass);
  addBox( 31.5, 6, -16, 0.5, 10, 16, matGlass);

  // Red carpet runner
  addDecoBox(0, 1.15, 12, 28, 0.08, 72, matCarpet); // Ground center
  addDecoBox(0, 10.05, -36, 108, 0.08, 16, matCarpet); // Mezzanine back
  addDecoBox(-44, 10.05, 0, 16, 0.08, 48, matCarpet); // Mezzanine sides
  addDecoBox( 44, 10.05, 0, 16, 0.08, 48, matCarpet);

  // Base trim
  [[-56, 1.4, 0, 1.5, 3, 100], [56, 1.4, 0, 1.5, 3, 100], [0, 1.4, -48, 112, 3, 1.5], [0, 1.4, 48, 112, 3, 1.5]]
    .forEach(a => addDecoBox(...a, matDarkWood)); // Ground
  [[-56, 11.5, 0, 1.5, 3, 100], [56, 11.5, 0, 1.5, 3, 100], [0, 11.5, -48, 112, 3, 1.5]]
    .forEach(a => addDecoBox(...a, matDarkWood)); // Mezzanine

  // Grand marble columns (floor to ceiling)
  [[ -44, 14, -36, 4, 28, 4],
   [  44, 14, -36, 4, 28, 4],
   [ -44, 14,  36, 4, 28, 4],
   [  44, 14,  36, 4, 28, 4],
   [ -44, 14,   0, 4, 28, 4],
   [  44, 14,   0, 4, 28, 4]]
    .forEach(a => { addBox(...a, matMarble); });

  // Gold capitals
  [[ -44, 26, -36, 4.6, 1.5, 4.6],
   [  44, 26, -36, 4.6, 1.5, 4.6],
   [ -44, 26,  36, 4.6, 1.5, 4.6],
   [  44, 26,  36, 4.6, 1.5, 4.6],
   [ -44, 26,   0, 4.6, 1.5, 4.6],
   [  44, 26,   0, 4.6, 1.5, 4.6]]
    .forEach(a => addDecoBox(...a, matGoldTrim));

  // Front desk counter (Ground level)
  addBox(0, 3.0, -42, 28, 4.0, 6, matDarkWood);
  addBox(0, 5.5, -42, 29, 1.0, 7, matMarble);

  // Lobby lounge areas (Sofas/Tables)
  addBox(-12, 2.5, 20, 8, 3, 12, matDarkWood); // Sofa L
  addBox( 12, 2.5, 20, 8, 3, 12, matDarkWood); // Sofa R
  addBox(0, 1.8, 20, 6, 1.5, 10, matMarble);  // Coffee table

  // Planters
  [[-28, 2.5, 38], [28, 2.5, 38], [-28, 2.5, 2], [28, 2.5, 2]].forEach(([x, y, z]) => {
    addBox(x, y, z, 5, 3, 5, matDarkWood); // Box
    addDecoBox(x, y+2, z, 4, 4, 4, matPlant); // Plant
  });

  // Elevator bank visual cover
  addDecoBox(-56, 5, 20, 2, 8, 16, matGoldTrim);
  addDecoBox(-55.5, 5, 20, 1, 7, 14, matGlass);
  addDecoBox( 56, 5, 20, 2, 8, 16, matGoldTrim);
  addDecoBox( 55.5, 5, 20, 1, 7, 14, matGlass);

  // Grand central chandelier
  addDecoBox(0, 24.0, 0, 6, 1.0, 6, matGoldTrim);
  addDecoBox(0, 22.0, 0, 14, 2.0, 14, matChandelier);
  addDecoBox(0, 19.5, 0, 8, 3.0, 8, matChandelier);
  addDecoBox(0, 17.0, 0, 4, 2.0, 4, matChandelier);

  // Wall sconces
  [[-56, 6, -18], [-56, 6, 0], [-56, 6, 38],
   [ 56, 6, -18], [ 56, 6, 0], [ 56, 6, 38],
   [-56, 16, -18], [-56, 16, 0], [-56, 16, 38],
   [ 56, 16, -18], [ 56, 16, 0], [ 56, 16, 38],
   [-18, 16, -48], [18, 16, -48]].forEach(([x, y, z]) => {
    addDecoBox(x, y, z, 1.0, 2.5, 1.0, matGoldTrim);
    addDecoBox(x, y+1.2, z, 1.4, 1.0, 1.4, matChandelier);
  });

  buildSkyDome('#0d0705', '#1a0f0a', '#060302', 720, 0xffe8c0);
}

function buildFletcherLights() {
  sceneAddArena(new THREE.AmbientLight(0xd4e2f4, 0.65));
  sceneAddArena(new THREE.HemisphereLight(0xe8f0ff, 0x303848, 0.35));
  ...
  const chandLight = new THREE.PointLight(0xffeeb0, 2.8, 80);
  chandLight.position.set(0, 16, 0);
  sceneAddArena(chandLight);

  // Mezzanine Lights
  [[-44, 16, -30], [44, 16, -30], [-44, 16, 20], [44, 16, 20], [0, 16, -36]].forEach(([x, y, z]) => {
    const l = new THREE.PointLight(0xffe8c8, 1.6, 45);
    l.position.set(x, y, z);
    sceneAddArena(l);
  });

  // Ground Floor Under-Mezzanine Lights
  [[-44, 7, -30], [44, 7, -30], [-44, 7, 20], [44, 7, 20], [0, 7, -36]].forEach(([x, y, z]) => {
    const l = new THREE.PointLight(0xdce8ff, 1.3, 35);
    l.position.set(x, y, z);
    sceneAddArena(l);
  });

  // Entry / Lounge Lights
  [[0, 7, 20], [0, 7, 36], [-28, 7, 40], [28, 7, 40]].forEach(([x, y, z]) => {
    const l = new THREE.PointLight(0xffdfb8, 1.5, 40);
    l.position.set(x, y, z);
    sceneAddArena(l);
  });
...
const SPAWN_POINTS = [
  ...
  // Map 6: DM-Fletcher
  [
    [0, 1.1, 0], [-40, 1.1, -35], [40, 1.1, -35], [-40, 1.1, 35], [40, 1.1, 35],
    [-45, 10.1, 0], [45, 10.1, 0], [0, 10.1, -40], [0, 1.1, 40],
    [-20, 1.1, -20], [20, 1.1, 20], [-20, 1.1, 20], [20, 1.1, -20],
  ],
  // Map 7: CTF-Face
  [
...
const CLASSIC_ITEM_SPAWNS = [
  ...
  // Map 6: DM-Fletcher
  [
    { type: 'weapon_shock', x: -30, y: 1.35, z: 0 },
    { type: 'weapon_rocket', x: 30, y: 1.35, z: 0 },
    { type: 'health', x: 0, y: 10.35, z: -30 }, // Moved up to mezzanine
    { type: 'health', x: 0, y: 1.35, z: 30 },
    { type: 'body_armor', x: -44, y: 10.35, z: 0 }, // Left mezzanine
    { type: 'shield_belt', x: 44, y: 10.35, z: 0 }, // Right mezzanine
    { type: 'ammo_cells', x: -35, y: 1.35, z: 30 },
    { type: 'ammo_rockets', x: 35, y: 1.35, z: 30 },
    { type: 'udamage', x: 0, y: 1.35, z: 0 },
  ],
  // Map 7: CTF-Face
  [
